...
class MainViewModel(private val application: Application) : ViewModel() {
    ...
    fun makeNetworkRequest() = viewModelScope.launch {
        withContext(Dispatchers.IO) {
            try {
                application.queueSharedInstance.add(
                    application.makeAGetRequest<Weather> {
                        ...
                        onFailure = { stackTrace, localizedMessage, message, networkTimeMs ->
                            _getWeather.value =
                                "$message took ${networkTimeMs}ms with $localizedMessage $stackTrace"
                        }
                    }
                )
